...
const saveProductDetails = (data: ProductDetails) => {
  if (!data.product?.name || !data.product.brand || !data.product.asin) {
...
const SYSTEM_PROMPT = `
  ...
  - Strictly follow the JSON output formet as in example
  ---
  ### **AVAILABLE TOOLS:**
...
async function main() {
  ...
  while (true) {
    try {
      ...
      if (!result) {
        console.error("Received an empty response from the model.");
        break;
      }
      ...
      try {
        ...
        console.log("AI RESPONSE:", cleanedResult);
        console.log(
          "asdasdasdasdasdasdasdasdasdasd",
          JSON.parse(cleanedResult)
        );
        call = JSON.parse(cleanedResult);
      } catch (error) {
      ...
      if (call.type === "output") {
        console.log("hello", call.message, call.product_details);
        break;
