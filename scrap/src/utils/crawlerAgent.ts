...
// const user="Hey, give me all valuable details of the product, I am giving you all data as string. This data is collected as 'document.documentElement.outerText'"
...
// tools
const saveProductDetails = (data: typeof SAMPLE_OBSERVATION) => {
  // Check if data contains necessary product details
  if (!data || !data.product || !data.product.asin) {
    return {
      message: "Invalid product data. Missing ASIN or product details.",
    };
  }

  const { asin } = data.product;

  // Check if the product already exists in the database (using ASIN as a unique identifier)
  if (db.has(asin)) {
    return {
